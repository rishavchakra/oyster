...
fn native_atoi(stack: *Stack, binding_map: *BindingMap, alloc: std.mem.Allocator) !void {
    ...
    if (arg_type == .char) {
        ...
        try stack.append(alloc, compiler.OpCode{ .int = compiler.Int.init(arg.char.val - 48) });
    }
...
fn native_itoa(stack: *Stack, binding_map: *BindingMap, alloc: std.mem.Allocator) !void {
    ...
    if (arg_type == .int) {
        ...
        try stack.append(alloc, compiler.OpCode{ .char = compiler.Char.init(@as(u8, @truncate(@abs(arg.int.val))) + '0') });
    }
    // TODO: handle typeerror case
}
